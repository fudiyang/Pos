...
public class Convert {
  ...
  ****************************************************************************/
   ...
   * @return 取反字符串
   */
  public static String notStoBtoS(String in)
	{
		String out = "";
		char[] xCha = in.toCharArray();
	    for(int i= 0; i< xCha.length; i++)
	    {
	    	byte a = Byte.valueOf(Integer.valueOf(Character.toString(xCha[i]), 16).toString());
		    out=out+Integer.toHexString(~a & 0x0f);
	    }
		return out;
	}
   ...
   * @return 异或结果
   */
  public static String xorStr(String xStr, String rStr)
	{
		String xor ="";
		
	    char[] lCha = xStr.toCharArray();
	    char[] rCha = rStr.toCharArray();
	   
	    for(int i= 0; i< lCha.length; i++)
	    {
	    	byte a = Byte.valueOf(Integer.valueOf(Character.toString(lCha[i]), 16).toString());
	    	byte b = Byte.valueOf(Integer.valueOf(Character.toString(rCha[i]), 16).toString());
		    xor = xor + Integer.toHexString(a ^ b);
	    }
		return xor;
	}
  
  	/**
	 * 已知一个加数以及和值，当此加数变化时候，和值相应变化，并取结果和值低两位 
	 * @param addend1 加数
	 * @param sumValue 变化前和值
	 * @param para 加数改变值
	 * @return 变化后和值
	 */
	public static String DataFollowChange(String addend1, String sumValue, String para)
	{
		int a = Integer.valueOf(addend1, 16);
		int b = Integer.valueOf(sumValue, 16);
		int c = Integer.valueOf(para, 16);
		
		String output = Integer.toHexString(c + b - a);
		if (output.length() == 1)
		{
			output = "0" + output;
		}
